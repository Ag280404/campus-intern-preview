...
const Dashboard: React.FC<DashboardProps> = ({ user, submissions, isAdmin }) => {
      ...
      <div className="space-y-8 animate-in fade-in duration-700 pb-12">
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-5 px-1 py-4">
          <div className="flex-1">
            <h2 className="text-[28px] font-black text-slate-900 tracking-tight leading-none">Campus Insights</h2>
          </div>
          <div className="flex gap-3 shrink-0 pt-1">
            <button 
              onClick={handleRefresh}
              className="flex items-center justify-center gap-2 px-5 py-2.5 bg-white border border-slate-200 rounded-[18px] text-[11px] font-black text-slate-600 hover:bg-slate-50 hover:text-swiggy-orange transition-all swiggy-shadow tracking-widest"
            >
            ...
            <a 
              ...
              className="flex items-center justify-center gap-2 px-6 py-2.5 bg-swiggy-orange text-white rounded-[18px] font-black text-[11px] hover:bg-[#E14A00] transition-all swiggy-shadow shadow-orange-100 tracking-widest"
            >
        ...
        <div className="bg-white rounded-[48px] p-10 md:p-14 swiggy-shadow border border-slate-100 relative overflow-hidden">
          ...
          <div className="inline-block bg-slate-900 text-white px-8 py-3.5 rounded-[20px] mb-12 shadow-xl shadow-slate-100">
            <h3 className="text-xl font-black tracking-tight">{campusName}</h3>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
             <div className="bg-slate-50/50 p-8 rounded-[32px] border border-slate-100 text-center space-y-1 transition-all hover:bg-white hover:swiggy-shadow">
                <p className="text-slate-400 font-bold text-[10px] tracking-widest">Assigned tasks</p>
             ...
             <div className="bg-swiggy-light/30 p-8 rounded-[32px] border border-swiggy-orange/10 text-center space-y-1 transition-all hover:bg-white hover:swiggy-shadow">
                <p className="text-swiggy-orange font-bold text-[10px] tracking-widest">Completed</p>
             ...
             <div className="bg-amber-50/30 p-8 rounded-[32px] border border-amber-200/50 text-center space-y-1 transition-all hover:bg-white hover:swiggy-shadow">
                <p className="text-amber-600 font-bold text-[10px] tracking-widest">In progress</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-16 max-w-2xl mx-auto">
             <div className="bg-slate-50/50 p-7 rounded-[32px] border border-slate-100 text-center space-y-1">
                <p className="text-slate-400 font-bold text-[10px] tracking-widest">Not started</p>
                <p className="text-5xl font-black text-slate-800 tracking-tighter">{stats.notStarted}</p>
             </div>
             <div className="bg-slate-50/50 p-7 rounded-[32px] border border-slate-100 text-center space-y-1">
                <p className="text-slate-400 font-bold text-[10px] tracking-widest">Pending verification</p>
                <p className="text-5xl font-black text-slate-800 tracking-tighter">{stats.notCompleted}</p>
             </div>
          </div>

          <div className="space-y-6">
            <h4 className="text-xl font-black text-slate-900 tracking-tight">Active Catalyst Workflow</h4>
            <div className="overflow-hidden border border-slate-100 rounded-[32px] shadow-sm">
              <table className="w-full text-left">
                <thead className="bg-slate-50/80 border-b border-slate-100">
                  <tr>
                    <th className="px-8 py-5 text-[10px] font-bold text-slate-400 tracking-widest">Id</th>
                    <th className="px-8 py-5 text-[10px] font-bold text-slate-400 tracking-widest">Task</th>
                    <th className="px-8 py-5 text-[10px] font-bold text-slate-400 text-center tracking-widest">Status</th>
                    <th className="px-8 py-5 text-[10px] font-bold text-slate-400 text-right tracking-widest">Progress</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-50">
                  {taskList.map((task, idx) => (
                    <tr key={idx} className="hover:bg-slate-50/50 transition-all group">
                      ...
                      <td className="px-8 py-5 text-sm font-black text-slate-800 tracking-tight">{task.name}</td>
                      <td className="px-8 py-5 text-center">
                        <span className={`px-3 py-1 rounded-full text-[9px] font-black tracking-widest ${task.color}`}>
                          {task.status}
                        </span>
                      </td>
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700 pb-12">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-5 px-1 py-4">
        <div className="flex-1">
          <h2 className="text-[28px] font-black text-slate-900 tracking-tight leading-none">Dashboard</h2>
        </div>
        <div className="flex gap-3 shrink-0 pt-1">
          <button 
            onClick={handleRefresh}
      ...
      <div className="relative w-full bg-white rounded-[48px] overflow-hidden swiggy-shadow border border-slate-50/80 flex flex-col group min-h-[720px] premium-card-shadow">
        <div className="absolute inset-0 bg-slate-50/50 flex items-center justify-center -z-10">
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="text-[10px] font-bold text-slate-400 mt-2 tracking-widest">Connecting Swiggy insight engine...</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 gap-5">
        <div className="bg-white p-7 rounded-[32px] border border-slate-100 swiggy-shadow flex gap-5 items-start">
          ...
          <div className="space-y-1">
            <h4 className="font-black text-slate-900 text-[13px] tracking-widest">Viewing Tip</h4>
