...
const InitiativeCard = ({ title, subtitle, description, image, badgeColor, href }: { title: string, subtitle: string, description: string, image: string, badgeColor: string, href: string }) => (
  ...
  >
    <div className="h-44 w-full overflow-hidden relative">
      ...
      <div className={`absolute top-5 left-5 ${badgeColor} text-white px-4 py-1.5 rounded-full text-[9px] font-bold shadow-lg uppercase tracking-widest`}>
        {subtitle}
      </div>
    </div>
    <div className="p-7 flex flex-col flex-1">
      <h5 className="text-xl font-black text-slate-900 mb-3 tracking-tight group-hover:text-swiggy-orange transition-colors uppercase">{title}</h5>
      <p className="text-xs text-slate-500 font-bold leading-relaxed flex-1 opacity-80 uppercase tracking-tight">
        {description}
      </p>
    </div>
  </a>
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-md animate-in fade-in duration-300">
      <div className="bg-white w-full max-w-lg rounded-[48px] overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300 border border-white/20">
        <div className="px-10 pt-10 pb-4 flex justify-between items-center">
          <h3 className="text-2xl font-black text-slate-900 tracking-tight uppercase">Distribute asset</h3>
...
const Profile: React.FC<ProfileProps> = ({ user, onUserUpdate }) => {
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-700 pb-20">
      ...
      <header>
        <h2 className="text-[32px] font-black text-slate-900 tracking-tight leading-none uppercase">PROFILE</h2>
      </header>
            ...
            {!isEditing ? (
              <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
                <div className="text-center md:text-left">
                  <h3 className="text-[36px] font-black text-slate-900 leading-none mb-3 tracking-tighter uppercase">{user.displayName}</h3>
                  <div className="inline-flex items-center gap-2 bg-swiggy-light text-swiggy-orange px-4 py-1.5 rounded-full font-bold text-[10px] mb-6 shadow-sm uppercase tracking-widest">
                    <MapPin size={12} strokeWidth={3} /> {campusName}
                  </div>
                  <div className="flex flex-wrap gap-3 justify-center md:justify-start">
                    <div className="flex items-center gap-2.5 text-slate-600 bg-slate-50/80 px-5 py-3 rounded-2xl border border-slate-100 font-bold text-xs shadow-sm uppercase tracking-tight">
                ...
                <button 
                  ...
                  className="px-8 py-3.5 bg-slate-50 border border-slate-200 rounded-[22px] text-[11px] font-black text-slate-500 hover:text-swiggy-orange hover:bg-white hover:swiggy-shadow transition-all self-center md:self-start active:scale-95 uppercase tracking-widest"
                >
                  Edit Profile
                </button>
              </div>
              ...
              <div className="space-y-7 animate-in fade-in slide-in-from-top-4 duration-500">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div className="space-y-2">
                    <label className="block text-[10px] font-bold text-slate-400 ml-3 uppercase tracking-widest">Display Name</label>
                    <input 
                      type="text"
                      value={editName}
                      onChange={(e) => setEditName(e.target.value)}
                      className="w-full px-6 py-4.5 bg-slate-50 border border-slate-200 rounded-[24px] outline-none focus:ring-4 focus:ring-swiggy-orange/5 focus:bg-white font-black text-slate-800 transition-all uppercase"
                    />
                  </div>
                  <div className="space-y-2">
                    <label className="block text-[10px] font-bold text-slate-400 ml-3 uppercase tracking-widest">Phone Number</label>
                    <input 
                      type="tel"
                      value={editPhone}
                ...
                <div className="space-y-2">
                  <label className="block text-[10px] font-bold text-slate-400 ml-3 uppercase tracking-widest">Email ID</label>
                  <input 
                    type="email"
                    value={editEmail}
                ...
                <div className="flex items-center justify-between p-6 bg-slate-50/80 rounded-[32px] border border-slate-100 shadow-inner">
                   <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="text-[13px] font-black text-slate-900 leading-none mb-1 uppercase tracking-tight">Directory Visibility</p>
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Allow peer catalysts to find your contact</p>
                      </div>
                   </div>
                   <button 
                      type="button"
                ...
                <div className="flex gap-4 justify-end pt-4">
                  <button 
                    ...
                    className="px-10 py-4 bg-swiggy-orange text-white rounded-[24px] text-xs font-black shadow-xl shadow-orange-100 flex items-center gap-3 hover:bg-[#E14A00] transition-all hover:-translate-y-0.5 uppercase tracking-widest"
                  >
                  ...
                  <button 
                    ...
                    className="px-10 py-4 bg-slate-100 text-slate-500 rounded-[24px] text-xs font-black hover:bg-slate-200 transition-all active:scale-95 uppercase tracking-widest"
                  >
                    Discard
                  </button>
                </div>
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 gap-8">
         <div className="bg-white p-10 rounded-[56px] swiggy-shadow border border-slate-50 text-center flex flex-col justify-between items-center group premium-card-shadow relative overflow-hidden">
            ...
            <div className="w-full relative">
              ...
              <h4 className="text-base font-black text-slate-900 mb-1.5 uppercase tracking-tight">Student Rewards asset</h4>
              <p className="text-[11px] text-slate-400 font-bold mb-10 tracking-tight uppercase tracking-widest">Campaign specific QR • Dynamic link mapping</p>
            ...
            <button 
              ...
              className="w-full bg-slate-900 text-white py-5 rounded-[26px] font-black text-[11px] flex items-center justify-center gap-3 transition-all hover:bg-black active:scale-95 shadow-xl shadow-slate-100 uppercase tracking-widest"
            >
         ...
         <div className="bg-white p-10 rounded-[56px] swiggy-shadow border border-slate-50 text-center flex flex-col justify-between items-center group premium-card-shadow relative overflow-hidden">
            ...
            <div className="w-full relative">
              ...
              <h4 className="text-base font-black text-slate-900 mb-1.5 uppercase tracking-tight">Campus Streaks asset</h4>
              <p className="text-[11px] text-slate-400 font-bold mb-10 tracking-tight uppercase tracking-widest">Real-time activation QR • Cloud-updated</p>
            ...
            <button 
              ...
              className="w-full bg-swiggy-orange text-white py-5 rounded-[26px] font-black text-[11px] flex items-center justify-center gap-3 transition-all hover:bg-[#E14A00] active:scale-95 shadow-xl shadow-orange-100 uppercase tracking-widest"
            >
              <Share2 size={20} strokeWidth={3} /> Distribute Asset
            </button>
         </div>
      ...
      <section className="bg-white rounded-[56px] p-10 md:p-14 swiggy-shadow border border-slate-50 premium-card-shadow">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-12">
          ...
          {!user.shareContactInfo && (
            <button 
              ...
              className="px-10 py-4 bg-swiggy-orange text-white rounded-[24px] font-black text-[11px] shadow-xl shadow-orange-100 transition-all hover:bg-[#E14A00] hover:-translate-y-0.5 uppercase tracking-widest"
            >
              Join Directory
            </button>
          )}
        ...
        {allPeers.length > 0 ? (
            ...
            {allPeers.map(peer => (
              <div key={peer.id} className="bg-slate-50/50 p-8 rounded-[40px] border border-slate-100 hover:bg-white hover:border-swiggy-orange/30 transition-all duration-500 group swiggy-shadow flex items-start gap-6">
                ...
                <div className="min-w-0">
                  <h5 className="font-black text-slate-900 truncate text-lg mb-1 tracking-tight group-hover:text-swiggy-orange transition-colors uppercase tracking-tight">{peer.displayName}</h5>
                  <div className="inline-flex items-center gap-1.5 text-[9px] text-swiggy-orange font-bold mb-4 bg-swiggy-light px-3 py-1 rounded-full uppercase tracking-widest">
                    <MapPin size={10} strokeWidth={3} /> {db.getCampusName(peer.campusId)}
                  </div>
                  <div className="space-y-2.5">
                    <a href={`mailto:${peer.email}`} className="flex items-center gap-3 text-[11px] font-bold text-slate-500 hover:text-swiggy-orange transition-colors group/link uppercase tracking-tight">
                      <Mail size={14} className="text-slate-300 group-hover/link:text-swiggy-orange" strokeWidth={2.5} /> {peer.email}
                    </a>
                    {peer.phoneNumber && (
                      <a href={`tel:${peer.phoneNumber}`} className="flex items-center gap-3 text-[11px] font-bold text-slate-500 hover:text-swiggy-orange transition-colors group/link uppercase tracking-tight">
          ...
          <div className="p-20 text-center bg-slate-50/50 rounded-[44px] border-2 border-dashed border-slate-200">
            ...
            <h4 className="font-black text-slate-900 text-xl tracking-tight uppercase tracking-tight">Syncing Network...</h4>
            <p className="text-[11px] text-slate-400 mt-3 font-bold uppercase tracking-widest">Catalyst directory populates as peers opt-in.</p>
          </div>
        )}
      </section>
      ...
      <section className="space-y-10 pt-6">
        <div className="flex items-center gap-4">
          ...
          <h3 className="text-[28px] font-black text-slate-900 tracking-tight leading-none uppercase tracking-tighter">Core Campus Missions</h3>
        </div>
